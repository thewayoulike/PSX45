...
import { editorialPages, toolPages } from '../config/editorialPages.js';
...
import { buildGuideArticleJsonLd, buildGuideBreadcrumbJsonLd, buildPageArticleJsonLd, buildPageBreadcrumbJsonLd, faqsForGuide, formatUpdated, guideUpdated, updatedIso, GUIDE_AUTHOR } from './guideSchema.js';
...
export function renderGuidePage(slug) {
  ...
  const faqs = faqsForGuide(slug, page.faqs);
  const faqHtml = faqs.length
    ? `<section id="faq"><h2>Common questions</h2>${faqs.map(([q, a]) => `<h3>${escape(q)}</h3><p>${escape(a)}</p>`).join('')}</section>`
    : '';
  const byline = `<p class="byline">By ${escape(GUIDE_AUTHOR)} · Updated ${escape(formatUpdated(guideUpdated(slug)))}</p>`;
  const body = `${byline}<div class="actions"><a class="button" href="/login">Start free</a><a class="button secondary" href="/guides">All guides</a></div><div class="layout"><aside class="contents"><strong>On this page</strong>${page.sections.map(([h], i) => `<a href="#section-${i + 1}">${escape(h)}</a>`).join('')}</aside><div class="sections">${sectionHtml}${faqHtml}${related ? `<section><h2>Related guides</h2><p>${related}${related ? ' · ' : ''}${howToUseLink}</p></section>` : ''}<a href="/contact">Need help? Contact support →</a></div></div>`;
  const head = `${buildGuideArticleJsonLd(page, slug)}${buildGuideBreadcrumbJsonLd(page, slug)}${buildGuideFaqJsonLd(faqs, `${SITE_URL}/guides/${slug}`)}`;
  return shell({
    title: page.title,
    description: page.description,
...
function toolForm(id) {
  const input = (name, label, required = true) =>
    `<label>${escape(label)}<input name="${name}" type="number" inputmode="decimal" step="any"${required ? ' required' : ''}></label>`;
  const fields = {
    'capital-gains': [
      input('quantity', 'Quantity'),
      input('buyPrice', 'Buy price'),
      input('sellPrice', 'Sell price'),
      input('buyFees', 'Buy fees (Rs)', false),
      input('sellFees', 'Sell fees (Rs)', false),
      `<label>Buy date<input name="acquired" type="date" required></label>`,
      `<label>Sell date<input name="sold" type="date" required></label>`,
      `<label>Your status<select name="onAtl"><option value="yes">On the Active Taxpayers List (filer)</option><option value="no">Not on the Active Taxpayers List (non-filer)</option></select></label>`,
    ],
    'fifo-vs-average': [
      input('q1', 'First lot quantity'),
      input('p1', 'First lot price'),
      input('q2', 'Second lot quantity'),
      input('p2', 'Second lot price'),
      input('sellQty', 'Shares sold'),
      input('sellPrice', 'Sale price'),
    ],
    'dividend-yield': [
      input('dividend', 'Annual cash dividend per share'),
      input('price', 'Price per share'),
    ],
    'trade-costs': [
      input('value', 'Trade value (Rs)'),
      input('commissionPct', 'Commission %'),
      input('salesTaxPct', 'Tax on commission %'),
      input('cdc', 'CDC amount (Rs)'),
    ],
    sip: [
      input('monthly', 'Monthly amount (Rs)'),
      input('annualReturnPct', 'Assumed annual return %'),
      input('years', 'Years'),
      input('initial', 'Amount you already have (Rs)', false),
    ],
    zakat: [
      input('cash', 'Cash and bank balances (Rs)', false),
      input('goldGrams', 'Gold you hold (grams)', false),
      input('goldPrice', 'Gold price per gram (Rs)', false),
      input('silverGrams', 'Silver you hold (grams)', false),
      input('silverPrice', 'Silver price per gram (Rs)', false),
      input('investments', 'Shares and funds you are treating as zakatable (Rs)', false),
      input('debts', 'Debts you are deducting (Rs)', false),
      `<label>Nisab measure<select name="nisabMetal"><option value="silver">Silver nisab (612.36 grams)</option><option value="gold">Gold nisab (87.48 grams)</option></select></label>`,
    ],
  };
  const inputs = (fields[id] || []).join('');
  return `<form id="${id}" class="calc">${inputs}<button type="submit">Calculate</button></form><p id="${id}-out" role="status"></p>`;
}

export function renderEditorialPage(pathname) {
  const page = editorialPages[pathname] || toolPages[pathname];
  if (!page) throw new Error(`Unknown public resource: ${pathname}`);
  const sectionHtml = page.sections.map((section, i) => {
    const [h, ...blocks] = section;
    const body = blocks.map((block) => renderGuideBlock(block)).join('');
    return `<section id="section-${i + 1}"><h2>${escape(h)}</h2>${body}</section>`;
  ...
  const faqHtml = faqs.length
    ? `<section id="faq"><h2>Common questions</h2>${faqs.map(([q, a]) => `<h3>${escape(q)}</h3><p>${escape(a)}</p>`).join('')}</section>`
    : '';
  const parent = pathname.startsWith('/tools') ? '/tools' : '/markets';
  const parentLabel = pathname.startsWith('/tools') ? 'All tools' : 'All markets';
  const byline = `<p class="byline">By ${escape(GUIDE_AUTHOR)} · Updated ${escape(formatUpdated(updatedIso(pathname)))}</p>`;
  const form = page.form ? toolForm(page.form) : '';
  const body = `${byline}<div class="actions"><a class="button" href="/login">Start free</a><a class="button secondary" href="${parent}">${parentLabel}</a></div><div class="layout"><aside class="contents"><strong>On this page</strong>${page.sections.map(([h], i) => `<a href="#section-${i + 1}">${escape(h)}</a>`).join('')}</aside><div class="sections">${form}${sectionHtml}${faqHtml}<a href="/guides">Read the guides →</a></div></div>`;
  const head = `${buildPageArticleJsonLd(page, pathname)}${buildPageBreadcrumbJsonLd(page, pathname)}${buildGuideFaqJsonLd(faqs, `${SITE_URL}${pathname}`)}${page.form ? '<script type="module" src="/calculators.js"></script>' : ''}`;
  return shell({
    title: page.h1,
    seoTitle: page.seoTitle || '',
    description: page.description,
...
export function resolvePublicHtml(pathname) {
  ...
  if (editorialPages[`/${path}`] || toolPages[`/${path}`]) return renderEditorialPage(`/${path}`);
