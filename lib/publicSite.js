...
import { publicVisualTheme } from './publicVisualTheme.js';
...
export function renderGuideHub() {
  ...
  const body = `<div class="actions"><a class="button" href="/login">Start free</a><a class="button secondary" href="/about">About PSX Tracker</a></div><div class="guide-list"><a href="/how-to-use"><strong>Start here: watch the 6-minute video guide</strong><span>Create a portfolio, set up your broker, record trades, and try AI and broker email scanning. Includes captions and chapter shortcuts.</span></a><a href="/how-it-works"><strong>Explore every feature</strong><span>The video and detailed written guide with screenshots.</span></a>${cards}</div>`;
  return shell({
...
export function renderGuidePage(slug) {
  ...
  const sectionHtml = page.sections.map((section, i) => {
    ...
    const body = blocks.map((block) => (
      Array.isArray(block)
        ? `<ul>${block.map((item) => `<li>${escape(item)}</li>`).join('')}</ul>`
        : `<p>${escape(block)}</p>`
    )).join('');
    return `<section id="section-${i + 1}"><h2>${escape(h)}</h2>${body}</section>`;
  }).join('');
  const body = `<div class="actions"><a class="button" href="/login">Start free</a><a class="button secondary" href="/guides">All guides</a></div><div class="layout"><aside class="contents"><strong>On this page</strong>${page.sections.map(([h], i) => `<a href="#section-${i + 1}">${escape(h)}</a>`).join('')}</aside><div class="sections">${sectionHtml}${related ? `<section><h2>Related guides</h2><p>${related}</p></section>` : ''}<a href="/contact">Need help? Contact support →</a></div></div>`;
  return shell({
    title: page.title,
    description: page.description,
