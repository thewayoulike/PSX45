...
import { guidePages, guideHub, guideHubClusters, guideHubStartHere } from '../config/guidePages.js';
...
export function renderGuideHub() {
  const startCards = guideHubStartHere.map((c) => guideCardHtml(c.href, c.title, c.blurb)).join('');
  ...
  const body = `<div class="actions"><a class="button" href="/login">Start free</a><a class="button secondary" href="/about">About PSX Tracker</a></div><section class="guide-cluster" id="start-here"><h2>Start here</h2><p>Watch the walkthrough, then use the shorter how-tos below.</p><div class="guide-list">${startCards}</div></section>${clusters}`;
  return shell({
