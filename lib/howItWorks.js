...
export function renderHowItWorksBody() {
  return `<div class="feature-guide">
    ...
    <nav class="feature-contents" aria-label="Feature guide contents"><strong>Jump to a feature</strong>${featureSections.map(s => `<a href="#guide-${s.id}">${escape(s.title)}</a>`).join('')}</nav>
