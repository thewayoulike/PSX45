...
import { fetchPsx } from './psxPortal.js';
...
export async function fetchPsxOhlc(symbol) {
  ...
  const res = await fetchPsx(HISTORICAL_URL, {
    ...
    await Promise.all(
      chunk.map(async (sym) => {
        try {
          ...
          const pakistanNow = new Date(Date.now() + 5 * 3600000);
          const open = pakistanNow.getUTCDay() > 0 && pakistanNow.getUTCDay() < 6 && pakistanNow.getUTCHours() >= 9 && pakistanNow.getUTCHours() < 16;
          const bar = new Date(last.time + 5 * 3600000);
          const sameDay = bar.getUTCFullYear() === pakistanNow.getUTCFullYear() && bar.getUTCMonth() === pakistanNow.getUTCMonth() && bar.getUTCDate() === pakistanNow.getUTCDate();
          if (open && !sameDay) return;
          out[sym] = last.close;
        } catch {
