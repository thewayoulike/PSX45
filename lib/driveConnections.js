...
function config() {
  ...
  if (!clientId) throw failure(503, 'Remembered Drive access needs the Google client ID in server settings.');
  if (!clientSecret) throw failure(503, 'Remembered Drive access needs GOOGLE_CLIENT_SECRET in server settings.');
  if (key.length !== 32) throw failure(503, 'DRIVE_TOKEN_ENCRYPTION_KEY must be a Base64-encoded 32-byte random key. Please correct the server setting.');
  let origin;
  try { origin = new URL(process.env.APP_URL || 'https://www.psx-tracker.com').origin; }
  catch { throw failure(503, 'Remembered Drive access needs a valid APP_URL in server settings.'); }
...
async function googleTokens(params, cfg) {
  ...
  if (!result.ok) {
    if (data.error === 'invalid_client' || data.error === 'unauthorized_client') throw failure(503, 'The Google OAuth client ID and client secret do not match an enabled web client. Please correct the server settings.');
...
export async function handleDriveConnection(req, res, body) {
  ...
  try {
    if (body.action === 'drive-config') {
      ...
      catch (error) { return res.status(200).json({ enabled: false, error: error.status ? error.message : 'Remembered Drive access is not configured yet.' }); }
    }
    const cfg = config();
    ...
    if (body.action === 'drive-status') {
      // Only report the connection belonging to this verified Google token.
      // This does not return credentials or trust a browser-supplied email.
      const bearer = String(req.headers.authorization || req.headers.Authorization || '').match(/^Bearer\s+(.+)$/i);
      if (!bearer) throw failure(401, 'Sign in with Google to check your Drive connection.');
      const google = await googleIdentity(bearer[1]);
      const row = await readConnection(google.email);
      return res.status(200).json({ connected: !!row && row.google_sub === google.sub });
    }
