...
export function isMufapBlockedPage(html) {
  ...
  const looksLikeNavTable =
    /group-link|fund-block|report date/i.test(html) || (html.match(/<td\b/gi) || []).length > 100;

  // Hard blocks: interstitial challenge pages (not embedded turnstile widgets).
  if (lower.includes('just a moment') || lower.includes('enable javascript and cookies')) {
    return true;
  }
  // Successful MUFAP pages embed turnstile (`challenge-platform` / `cf-chl`) with a real table.
  if (looksLikeNavTable) return false;
  if (html.length < 1500) return true;
  if (lower.includes('cf-chl') || lower.includes('challenge-platform')) return true;
  return false;
}

/** AMC section header: colspan cell with .group-link (modern MUFAP HTML). */
function amcFromHtmlHeaderCells(cells) {
  if (cells.length !== 1) return null;
  const raw = cleanCell(cells[0]);
  if (!raw || raw.length < 8) return null;
  if (!looksLikeAmc(raw)) return null;
...
export function parseMufapNavHtml(html) {
  ...
  // 1) Real HTML <tr> rows (classic MUFAP page + modern group-link layout)
  ...
  while ((rowMatch = rowRegex.exec(html)) !== null) {
    ...
    const amcHeader = amcFromHtmlHeaderCells(cells) || amcFromHeaderCells(cells);
    if (amcHeader) {
