/**
 * Call pypsx-toolkit via Vercel Python (/api/pypsx) or local Python scripts.
 */

import { spawn } from 'child_process';
import { fileURLToPath } from 'url';
import path from 'path';
import { validateMarketQuery } from './marketLimits.js';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const COMPANY_SCRIPT = path.join(ROOT, 'scripts', 'pypsx-company-info.py');
const ANALYSIS_SCRIPT = path.join(ROOT, 'scripts', 'pypsx-chart-analysis.py');
const INTRADAY_SCRIPT = path.join(ROOT, 'scripts', 'pypsx-intraday.py');
const CLI_SCRIPT = path.join(ROOT, 'scripts', 'pypsx-cli.py');
const PYTHON = process.platform === 'win32' ? 'python' : 'python3';

function runPythonScript(script, args) {
  return new Promise((resolve, reject) => {
    const proc = spawn(PYTHON, [script, ...args], {
      cwd: ROOT,
      windowsHide: true,
      env: process.env,
    });
    const timer = setTimeout(() => { proc.kill(); reject(new Error('Market-data request timed out')); }, 35000);

    let stdout = '';
    let stderr = '';
    proc.stdout.on('data', (c) => {
      stdout += c;
    });
    proc.stderr.on('data', (c) => {
      stderr += c;
    });
    proc.on('error', (error) => { clearTimeout(timer); reject(error); });
    proc.on('close', (code) => {
      clearTimeout(timer);
      if (code !== 0) {
        reject(new Error(stderr.trim() || stdout.trim() || `pypsx exit ${code}`));
        return;
      }
      try {
        resolve(JSON.parse(stdout.trim()));
      } catch {
        reject(new Error(`Invalid JSON from pypsx: ${stdout.slice(0, 200)}`));
      }
    });
  });
}

function getVercelBaseUrl() {
  const candidates = [
    process.env.VERCEL_URL,
    process.env.VERCEL_BRANCH_URL,
    process.env.VERCEL_PROJECT_PRODUCTION_URL,
  ].filter(Boolean);
  for (const raw of candidates) {
    if (raw.startsWith('http')) return raw.replace(/\/$/, '');
    return `https://${raw}`.replace(/\/$/, '');
  }
  return '';
}

async function fetchPypsxHttp(mode, params) {
  const qs = new URLSearchParams({ mode, ...params });
  const base = getVercelBaseUrl();
  if (!base) throw new Error('Vercel base URL unavailable for pypsx HTTP fetch');
  const url = `${base}/api/pypsx?${qs.toString()}`;
  const res = await fetch(url, { headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(40000) });
  const json = await res.json().catch(() => ({}));
  // quotes may include partial errors but still return quotes{}
  if (!res.ok || (json?.error && !(mode === 'quotes' && json?.quotes))) {
    throw new Error(json?.error || `pypsx HTTP ${res.status}`);
  }
  return json;
}

/**
 * @param {'company'|'dividends'|'analysis'|'intraday'|'quote'|'quotes'|'indices'} mode
 * @param {Record<string, string>} params
 */
export async function fetchPypsxToolkit(mode, params) {
  const invalid = validateMarketQuery(params);
  if (invalid) throw new Error(invalid);
  if (process.env.VERCEL) {
    return fetchPypsxHttp(mode, params);
  }

  if (mode === 'company') {
    const symbol = String(params.symbol || params.company || '').trim();
    const data = await runPythonScript(COMPANY_SCRIPT, [symbol]);
    if (data?.error) throw new Error(data.error);
    return data;
  }

  if (mode === 'dividends') {
    const symbol = String(params.symbol || '').trim();
    const data = await runPythonScript(CLI_SCRIPT, ['dividends', symbol]);
    if (data?.error) throw new Error(data.error);
    return data;
  }

  if (mode === 'analysis') {
    const symbol = String(params.symbol || params.analysis || '').trim();
    const period = String(params.period || '6mo');
    const data = await runPythonScript(ANALYSIS_SCRIPT, [symbol, period]);
    if (data?.error) throw new Error(data.error);
    return data;
  }

  if (mode === 'intraday') {
    const symbol = String(params.symbol || params.intraday || '').trim();
    const interval = String(params.interval || '5m');
    const period = String(params.period || '5d');
    const data = await runPythonScript(INTRADAY_SCRIPT, [symbol, interval, period]);
    if (data?.error) throw new Error(data.error);
    return data;
  }

  if (mode === 'quote') {
    const symbol = String(params.symbol || params.quote || '').trim();
    const data = await runPythonScript(CLI_SCRIPT, ['quote', symbol]);
    if (data?.error) throw new Error(data.error);
    return data;
  }

  if (mode === 'quotes') {
    const symbols = String(params.symbols || params.symbol || '').trim();
    const data = await runPythonScript(CLI_SCRIPT, ['quotes', symbols]);
    if (data?.error && !data?.quotes) throw new Error(data.error);
    return data;
  }

  if (mode === 'indices') {
    const name = String(params.name || params.index || '').trim();
    const data = await runPythonScript(CLI_SCRIPT, ['indices', name]);
    if (data?.error) throw new Error(data.error);
    return data;
  }

  throw new Error(`Unknown pypsx mode: ${mode}`);
}
