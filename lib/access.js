...
export function computeAccess(row) {
  ...
  // Only a persisted, valid approval date establishes a trial. Legacy undated
  // approvals retain Free access; administrators may grant a dated trial explicitly.
  const trialEnd = Number.isFinite(approvedAt) ? approvedAt + TRIAL_DAYS * DAY : null;
  ...
  return {
    ...
    lifetime: false,
    accessUntil: row.access_until || null, trialEnds: trialEnd === null ? null : new Date(trialEnd).toISOString(),
    daysLeft: 0,
    quotas: FREE_QUOTAS,
  };
}
