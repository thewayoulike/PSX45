...
export type ReportedDividend = { value: number; retrievedAt: string };
...
export function readResearch(raw?: unknown): Research {
  const result: Research = { version: 2, inputs: emptyInputs(), sources: {}, reportedDividend: null, savedAt: null, needsReview: false };
  ...
  const dividend = finiteNumber(record.reportedDividend?.value);
  const dividendDate = date(record.reportedDividend?.retrievedAt);
  if (modern && dividend !== null && dividend >= 0 && dividendDate) result.reportedDividend = { value: dividend, retrievedAt: dividendDate };
...
export function calculateFairValue(inputs: FairValueInputs) {
  ...
  const peValue = positive(n.eps) && positive(n.fairPE) ? calculated(n.eps * n.fairPE) : unavailable('Enter positive EPS and your own positive target P/E.');
  const dividendValue = nonNegative(n.expectedDiv) && positive(n.requiredReturn) ? calculated(n.expectedDiv / (n.requiredReturn / 100)) : unavailable('Enter an annual dividend of zero or more and a required return above 0%.');
  ...
  const forwardPE = positive(n.price) && positive(n.eps) && n.cagr !== null && n.cagr > -100 ? calculated(n.price / (n.eps * (1 + n.cagr / 100))) : unavailable('Requires positive price and EPS, and expected earnings growth above −100%.');
