...
it('never reloads during the save debounce or with a pending backup, and stops reload loops', () => {
  ...
  setUnsavedLocalChanges(true); expect(recoverMissingChunk(error)).toBe(false);
  setUnsavedLocalChanges(false); vi.stubGlobal('localStorage', { length: 1, key: () => 'psx_pending_cloud_v1:a' });
  expect(recoverMissingChunk(error)).toBe(false);
  vi.stubGlobal('localStorage', { length: 0 }); expect(recoverMissingChunk(error)).toBe(true); expect(recoverMissingChunk(error)).toBe(false);
