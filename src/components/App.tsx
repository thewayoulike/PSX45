...
const App: React.FC = () => {
  ...
  useEffect(() => {
      ...
      Object.entries(txsByKey).forEach(([key, txs]) => {
          ...
          // Mutual funds use AVERAGE COST, not FIFO. The engine is extracted to a
          // pure, unit-tested function (see utils/fundHoldings). Stocks keep the
          // FIFO engine below.
          if (isFundTicker(ticker)) {
              const { holding, realized } = computeFundBucketAverageCost(
                  ticker,
                  brokerName,
                  txs,
                  fundConversionMap,
                  {
                      ...
                      isFundPortfolio: isFundPort,
                  }
              );
              realized.forEach(r => tempRealized.push(r));
              if (holding) tempHoldings[key] = holding;
              return;
          }
  ...
  return (
    ...
    <div data-mobile-view={currentView} className="mobile-layout-app flex flex-col h-[100dvh] bg-[#f3f6f2] text-slate-900 font-sans selection:bg-emerald-200 dark:bg-[#0a0a0a] dark:text-slate-100 dark:selection:bg-emerald-900 overflow-hidden pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
      ...
      {showAddModal && <Suspense fallback={<div role="status" className="fixed bottom-6 right-6 z-50 rounded-lg bg-white p-4 shadow-lg text-slate-900">Opening tool…</div>}>
          ...
          importHistoryTransactions={transactions}
          portfolioId={currentPortfolioId}
