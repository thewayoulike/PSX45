...
const App: React.FC = () => {
  ...
  // Scanner State (Persisted per Portfolio)
  ...
  useEffect(() => {
    ...
    sortedTx.forEach(tx => {
      // Exclude special cash/fee types from HOLDINGS calculation
      if (tx.type === 'DEPOSIT' || tx.type === 'WITHDRAWAL' || tx.type === 'ANNUAL_FEE') return;
    ...
    displayedTransactions.forEach(t => {
        ...
        if (t.type === 'DIVIDEND') {
            ...
            totalCGT += (t.price * t.quantity);
             ...
             // Treat Annual Fee as a withdrawal from account value
             totalWithdrawals += t.price;
        } else {
