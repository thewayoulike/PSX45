...
export const AlgoTerminal: React.FC<AlgoTerminalProps> = ({ isOpen, onClose, defaultTicker }) => {
  ...
  // Targets & Ranges (Heuristics matching the screenshot style)
  ...
  const distSup = ((currentPrice - support) / currentPrice) * 100;

  // Short Term Targets (Resistance Band)
  const tpMin = resistance;
  const tpMax = resistance + (atr * 0.5);
  const profitPot = ((tpMin - currentPrice) / currentPrice) * 100;

  // Stoploss (Support Band)
  const slMin = support - (atr * 0.5);
  const slMax = support;

  // Long Term Targets (R2)
  const longTpMin = pivots ? pivots.r2 : currentPrice * 1.05;
  const longTpMax = longTpMin + atr;
  const longProfitPot = ((longTpMin - currentPrice) / currentPrice) * 100;

  // Risk Management
  const trailingSL = currentPrice - (2 * atr); // 2x ATR Trailing
  const buyZoneMin = support;
  const buyZoneMax = support + (atr * 0.2);
        ...
        <div className="flex-1 bg-[#313338] overflow-y-auto custom-scrollbar p-6">
            ...
            {analyzedTicker && latest && pivots && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="bg-[#2b2d31] rounded-lg p-4 border-l-4 border-[#5865F2]">
                            ...
                            <div className="space-y-2 text-xs">
                                ...
                                <div className="flex justify-between">
                                    <span className="text-[#949ba4]">Potential Profit</span>
                                    <span className="text-[#facc15] font-bold">{longProfitPot.toFixed(2)}%</span>
                                </div>
                                <div className="flex justify-between">
                    ...
                    {/* Chart Section */}
                    <div className="bg-[#111214] border border-[#1e1f22] rounded-lg p-4 relative">
                        <div className="flex justify-between items-center mb-2 px-2">
                            <span className="text-xs font-bold text-[#949ba4]">PRICE ACTION</span>
                            <div className="flex gap-3 text-[10px]">
                                <span className="flex items-center gap-1 text-[#facc15]"><div className="w-2 h-2 rounded-full bg-[#facc15]"></div> Price</span>
                                <span className="flex items-center gap-1 text-green-500"><div className="w-2 h-2 rounded-full bg-green-500"></div> TP Range</span>
                                <span className="flex items-center gap-1 text-rose-500"><div className="w-2 h-2 rounded-full bg-rose-500"></div> SL Range</span>
                            </div>
                        </div>
                        <div className="h-48 w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={data.slice(-30)}>
                                    ...
                                    {/* Target Lines */}
                                    <ReferenceLine y={tpMin} stroke="#22c55e" strokeDasharray="3 3" />
