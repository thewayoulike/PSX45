...
import { Bell, Loader2, CheckCircle2, AlertCircle, Target, TrendingUp, Search } from 'lucide-react';
...
export const AlertsPage: React.FC<AlertsPageProps> = ({ holdings, currentPrices }) => {
  ...
  // Get unique holdings for the quick-select list
  ...
  const handleSetAlert = async () => {
    ...
    try {
      ...
      // If we don't have a current price (custom ticker), default to ABOVE just in case
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="mt-4 space-y-6">
              ...
              <div className="relative">
                  ...
                  <div className="relative">
                      ...
                      <input 
                          ...
                          className="w-full pl-9 pr-3 py-2.5 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 font-bold dark:text-slate-200 uppercase"
                      />
                  </div>
              </div>
              ...
              <div className="relative">
                  ...
                  <div className="relative">
                      ...
                      <input 
                          ...
                          className="w-full pl-9 pr-3 py-2.5 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 font-mono font-bold dark:text-slate-200"
                      />
                  </div>
              </div>
          </div>
          ...
          <div className="pt-2">
              <button 
                  ...
                  disabled={status === 'loading' || !target || !ticker}
                  className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/20"
              >
              ...
              {message && (
                  <div className={`mt-4 flex items-center justify-center gap-1.5 text-sm font-bold p-3 rounded-xl border ${
                      status === 'success' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' 
                      : status === 'error' ? 'bg-rose-50 text-rose-500 border-rose-200' 
      ...
      <div className="bg-blue-50/50 dark:bg-blue-900/20 p-4 rounded-2xl border border-blue-100 dark:border-blue-800 text-xs text-blue-700 dark:text-blue-300">
          ...
          <em>(Note: Works on Android, Windows, and macOS. iOS requires the app to be "Added to Home Screen").</em>
      </div>
    </div>
  );
};
