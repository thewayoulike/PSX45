...
export const FundProfile: React.FC<FundProfileProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-50 dark:bg-[#0a0a0a] overflow-y-auto animate-in slide-in-from-right duration-300">
      ...
      {/* HEADER — same shell as stock TickerProfile */}
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md sticky top-0 z-40 border-b border-slate-200/60 dark:border-slate-800/60 px-4 sm:px-6 lg:px-8 xl:px-10 2xl:px-12 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-start gap-4 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <h1 className="text-3xl font-display font-black text-slate-900 dark:text-white tracking-tight truncate">
              {fundName}
            </h1>
            ...
            <div className="flex flex-col gap-2 mt-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {category && (
                  <span className="text-[9px] leading-none px-2 py-1 rounded-md border font-bold uppercase tracking-wider shadow-sm bg-indigo-50 text-indigo-600 border-indigo-200/60 dark:bg-indigo-500/10 dark:text-indigo-400 dark:border-indigo-500/20">
                    {category}
                  </span>
                )}
                <span className="text-[9px] leading-none px-2 py-1 rounded-md border font-bold uppercase tracking-wider shadow-sm bg-emerald-50 text-emerald-600 border-emerald-200/60 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20">
                  Mutual Fund
                </span>
              </div>
            </div>
          ...
          {stats.units > 0.0001 && (
            <div className="hidden sm:block">
              ...
              <div className="text-xl font-bold text-slate-900 dark:text-slate-100 font-mono tabular-nums">
                Rs. {fmtFundNav(currentNav)}
              </div>
              {lastUpdated && (
                <div className="text-[9px] text-slate-400 tabular-nums">{new Date(lastUpdated).toLocaleDateString()}</div>
              )}
            </div>
          )}
      ...
      <div className="w-full min-w-0 px-4 sm:px-6 lg:px-8 xl:px-10 2xl:px-12 py-6 md:py-8 space-y-8">
        ...
        {/* UNITS / VALUE CHART — fund stand-in for StockChart */}
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 rounded-3xl p-4 md:p-6 shadow-card dark:shadow-card-dark">
          <div className="flex items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-xl flex items-center justify-center border border-emerald-100 dark:border-emerald-500/20">
                <TrendingUp size={16} />
              </div>
              <div>
                <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">Units &amp; Value Over Time</h3>
                <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Fund activity · valued at current NAV</p>
              </div>
            </div>
            {stats.units > 0.0001 && (
              <div className="text-right hidden sm:block">
                <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Market Value</div>
                <div className="font-mono font-bold text-slate-900 dark:text-slate-100 tabular-nums">{rs(stats.marketValue)}</div>
              </div>
            )}
          </div>
          {balanceSeries.length > 1 ? (
            <ResponsiveContainer width="100%" height={280}>
              <AreaChart data={balanceSeries} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
                <defs>
                  <linearGradient id="fundUnitsFill" x1="0" y1="0" x2="0" y2="1">
                ...
                <Tooltip
                  ...
                  formatter={(v: number, name: string) =>
                    name === 'value'
                      ? [rs(v), 'Value'] as [string, string]
                      : [fmtFundUnits(v), 'Units'] as [string, string]
                  }
                />
                <Area type="stepAfter" dataKey="units" stroke="#10b981" strokeWidth={2.5} fill="url(#fundUnitsFill)" name="units" />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
            <div className="h-[280px] flex items-center justify-center text-sm text-slate-400">Not enough activity to chart.</div>
          )}
        </div>

        {/* STAT CARDS — same grid / labels as stock profile */}
              ...
              {stats.units > 0.0001 ? (
                <div className="space-y-6">
                  <div className="flex justify-between items-end">
                    <div>
                      <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Open Units</div>
                      <div className="text-3xl font-display font-black text-slate-900 dark:text-white tabular-nums">
                        {fmtFundUnits(stats.units)}
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <div className={`text-xs font-bold font-mono tabular-nums ${isUnrealizedProfit ? 'text-emerald-600/80 dark:text-emerald-400/80' : 'text-rose-600/80 dark:text-rose-400/80'}`}>
                        {isUnrealizedProfit ? '+' : ''}{stats.unrealizedPct.toFixed(2)}% ROI
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-5 border-t border-slate-100 dark:border-slate-800">
                    <div>
                      <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Invested</div>
                      <div className="font-mono font-bold text-slate-900 dark:text-slate-100 tabular-nums">{rs(stats.invested)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Current NAV</div>
                      <div className="font-mono font-bold text-slate-900 dark:text-slate-100 tabular-nums">Rs. {fmtFundNav(currentNav)}</div>
                    </div>
                  </div>
                </div>
              ) : (
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/60 dark:border-slate-800/60 shadow-card dark:shadow-card-dark relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-6">
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center border shadow-sm ${realizedStats.unitsOut > 0 ? (isRealizedProfit ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-500/20' : 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-100 dark:border-rose-500/20') : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700'}`}>
                  <CheckCircle size={18} />
                </div>
                <h3 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Realized (Closed)</h3>
              </div>

              {realizedStats.unitsOut > 0 ? (
                <div className="space-y-6">
                  <div className="flex justify-between items-end">
                    <div>
                      <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Units Out</div>
                      <div className="text-3xl font-display font-black text-slate-900 dark:text-white tabular-nums">
                        {fmtFundUnits(realizedStats.unitsOut)}
                      </div>
                    </div>
                    <div className="text-right">
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-5 border-t border-slate-100 dark:border-slate-800">
                    <div>
                      <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">W. Avg Cost</div>
                      <div className="font-mono font-bold text-slate-900 dark:text-slate-100 tabular-nums">Rs. {fmtFundNav(realizedStats.avgBuy)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">W. Avg Sell</div>
                      <div className="font-mono font-bold text-slate-900 dark:text-slate-100 tabular-nums">Rs. {fmtFundNav(realizedStats.avgSell)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Proceeds</div>
                      <div className="font-mono font-bold text-slate-900 dark:text-slate-100 tabular-nums">{rs(realizedStats.proceeds)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Cost</div>
                      <div className="font-mono font-bold text-slate-900 dark:text-slate-100 tabular-nums">{rs(realizedStats.cost)}</div>
                    </div>
                  </div>
                </div>
              ) : (
            ...
            <div className="relative z-10">
              ...
              <div className="space-y-2">
                <div className="text-3xl font-display font-black text-indigo-600 dark:text-indigo-400 tabular-nums">
                  {rsSigned(incomeStats.net)}
                </div>
                <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                  Net Dividends Collected
                  {incomeStats.reinvestUnits > 0 ? ` · ${fmtFundUnits(incomeStats.reinvestUnits)} units reinvested` : ''}
                </div>
              </div>
            </div>
          </div>
        ...
        {/* FUND LEDGER — stock Transaction History layout + fund P&L columns */}
            ...
            <table className="w-full text-left text-sm whitespace-nowrap min-w-[1000px] border-collapse">
              <thead className="bg-slate-50/95 dark:bg-slate-900/95 backdrop-blur-md text-[10px] uppercase text-slate-500 dark:text-slate-400 font-bold tracking-widest border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-6 py-3.5 text-right">NAV</th>
                  <th className="px-6 py-3.5 text-right">Net Amount</th>
                  <th className="px-6 py-3.5 text-right">Realized</th>
                  <th className="px-6 py-3.5 text-right">Unrealized</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                {rows.length === 0 ? (
                  ...
                  return (
                    <tr key={tx.id || i} className="even:bg-slate-50/50 dark:even:bg-slate-800/20 hover:bg-slate-100/60 dark:hover:bg-slate-800/60 transition-colors group">
                      <td className="px-6 py-3.5 text-slate-500 dark:text-slate-400 font-mono text-xs tabular-nums">{tx.date}</td>
                      ...
                      <td className="px-6 py-3.5 text-right font-mono font-bold text-slate-900 dark:text-slate-100 tabular-nums">{showUnits ? `${isOutflow(tx) ? '−' : '+'}${fmtFundUnits(tx.quantity)}` : '—'}</td>
                      <td className="px-6 py-3.5 text-right text-slate-600 dark:text-slate-400 font-mono tabular-nums">{showUnits ? fmtFundNav(tx.price) : '—'}</td>
                      <td className={`px-6 py-3.5 text-right font-bold font-mono tabular-nums ${net > 0 ? 'text-emerald-600 dark:text-emerald-400' : net < 0 ? 'text-rose-500 dark:text-rose-400' : 'text-slate-400'}`}>{net === 0 ? '—' : `${net >= 0 ? '+' : ''}${f0(net)}`}</td>
                      <td className={`px-6 py-3.5 text-right font-bold font-mono tabular-nums ${realized == null ? 'text-slate-400' : realized >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-500 dark:text-rose-400'}`}>{realized == null ? '—' : rsSigned(realized)}</td>
                      <td className={`px-6 py-3.5 text-right font-mono tabular-nums ${balance <= 0.0001 ? 'text-slate-400' : unrealized >= 0 ? 'text-emerald-600/90 dark:text-emerald-400/90' : 'text-rose-500/90 dark:text-rose-400/90'}`}>{balance <= 0.0001 ? '—' : rsSigned(unrealized)}</td>
                      <td className="px-6 py-3.5 text-right font-mono font-bold text-slate-700 dark:text-slate-200 tabular-nums">{fmtFundUnits(balance)}</td>
                    </tr>
                  );
                })}
              </tbody>
