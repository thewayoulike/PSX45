...
import { CheckCircle2, DownloadCloud, Loader2, Paperclip } from 'lucide-react';
import type { EmailImportStatus } from '../utils/emailImportHistory';

export function EmailAttachmentButton({ filename, size, status, disabled, onSelect }: {
  filename: string; size: number; status?: EmailImportStatus; disabled: boolean; onSelect: () => void;
}) {
  ...
  return (
    <button type="button" onClick={onSelect} disabled={disabled}
      ...
      <span className="flex flex-1 items-center gap-2 min-w-0 basis-48">
        ...
        <span className="text-[10px] text-slate-400 shrink-0">({Math.round(size / 1024)} KB)</span>
      </span>
