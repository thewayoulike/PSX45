...
export const ApiKeyManager: React.FC<ApiKeyManagerProps> = ({
  ...
  return (
    <div className="mobile-dialog-overlay mobile-dialog-full fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[70] flex items-start justify-center p-4 pt-20 md:pt-32 transition-opacity">
      <div role="dialog" aria-modal="true" aria-label="API Keys" className="mobile-dialog bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/60 rounded-3xl shadow-card dark:shadow-card-dark w-full max-w-md overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
          {!isDriveConnected ? (
            <div className="bg-amber-50/50 dark:bg-amber-500/10 border border-amber-200/60 dark:border-amber-500/20 rounded-2xl p-4 flex items-start gap-3 shadow-sm">
              ...
              <div>
                ...
                <p className="text-xs font-medium text-amber-700/80 dark:text-amber-400/80 mt-1 leading-snug">
                  Connect Google Drive to save your Gemini key on your Drive backup (your key stays yours).
                </p>
              </div>
            </div>
          ) : (
            <div className="bg-emerald-50/50 dark:bg-emerald-500/10 border border-emerald-200/60 dark:border-emerald-500/20 rounded-2xl p-4 flex items-start gap-3 shadow-sm">
              ...
              <div>
                <h4 className="font-bold text-emerald-800 dark:text-emerald-300 text-sm">Your Gemini key</h4>
                <p className="text-xs font-medium text-emerald-700/80 dark:text-emerald-400/80 mt-1 leading-snug">
                  Stored on this device and backed up to your Google Drive when sync completes. PSX Tracker does not keep your key on our servers.
                </p>
              </div>
            </div>
          )}
