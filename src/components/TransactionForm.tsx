  ...
  useEffect(() => {
    if (isOpen) {
        ...
        if (editingTransaction) {
            ...
            setTicker(''); setQuantity(''); setPrice(''); setCommission(''); setTax(''); setCdcCharges(''); setOtherFees(''); setNotes(''); if (savedScannedTrades.length > 0) {} else { setMode('MANUAL'); } setIsAutoCalc(true); setDate(new Date().toISOString().split('T')[0]); setCgtProfit(''); setHistAmount(''); setHistTaxType('AFTER_TAX'); setCategory('ADJUSTMENT'); setScanError(null); setSelectedFile(null); setEmailMessages([]); setEmailQuery(''); setEmailSender(''); if (portfolioDefaultBrokerId) setSelectedBrokerId(portfolioDefaultBrokerId);
        }
    }
