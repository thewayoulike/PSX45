...
// ... helper functions normalizeDate and getRowValue ... (same as before)
...
export const TransactionForm: React.FC<TransactionFormProps> = ({ 
  ...
  // ... (calculateFeesForTrade, handleAutoFillFees, updateScannedTrades, scanTotals, handleSelectAttachment, getHoldingQty, handleDownloadTemplate, handleManualSubmit, handleFileSelect, handleImportFile, handleProcessScan, toggleScanSelection, toggleSelectAll, getTradeCost, addSingleTrade, handleAcceptTrade, handleAcceptSelected, updateSingleScannedTrade, getFileIcon, getThemeColor - standard functions) ...
  // [Code omitted for brevity, logic is unchanged from previous working version]
  const updateScannedTrades = (trades: EditableTrade[]) => { if (onSaveScannedTrades) onSaveScannedTrades(trades); };
  const scanTotals = useMemo(() => { let totalBuy = 0; let totalSell = 0; savedScannedTrades.forEach(t => { const val = Number(t.quantity) * Number(t.price); const fees = (Number(t.commission)||0) + (Number(t.tax)||0) + (Number(t.cdcCharges)||0) + (Number(t.otherFees)||0); if (t.type === 'BUY') totalBuy += (val + fees); else if (t.type === 'SELL') totalSell += (val - fees); }); return { totalBuy, totalSell, net: totalSell - totalBuy }; }, [savedScannedTrades]);
  const calculateFeesForTrade = (tradeType: string, qty: number, prc: number, brokerId: string) => { if (qty <= 0 || prc <= 0) return { commission: 0, tax: 0, cdcCharges: 0 }; const gross = qty * prc; let estComm = 0; let estTax = 0; let estCdc = 0; if (tradeType === 'DIVIDEND') { estTax = gross * 0.15; } else { const currentBroker = brokers.find(b => b.id === brokerId); if (currentBroker) { if (currentBroker.commissionType === 'PERCENTAGE') estComm = gross * (currentBroker.rate1 / 100); else if (currentBroker.commissionType === 'FIXED') estComm = currentBroker.rate1; else if (currentBroker.commissionType === 'PER_SHARE') estComm = qty * currentBroker.rate1; else if (currentBroker.commissionType === 'HIGHER_OF') { const pct = gross * (currentBroker.rate1 / 100); const fixed = qty * (currentBroker.rate2 || 0); estComm = Math.max(pct, fixed); } else if (currentBroker.commissionType === 'SLAB' && currentBroker.slabs) { const slab = currentBroker.slabs.find(s => prc >= s.min && prc <= s.max); let slabComm = 0; if (slab) { if (slab.type === 'FIXED') slabComm = qty * slab.rate; else if (slab.type === 'PERCENTAGE') slabComm = gross * (slab.rate / 100); } if (currentBroker.rate1 && currentBroker.rate1 > 0) { const pctComm = gross * (currentBroker.rate1 / 100); estComm = Math.max(slabComm, pctComm); } else { estComm = slabComm; } } const taxRate = (currentBroker.sstRate / 100); estTax = estComm * taxRate; const cdcType = currentBroker.cdcType || 'PER_SHARE'; const cdcRate = currentBroker.cdcRate !== undefined ? currentBroker.cdcRate : 0.005; if (cdcType === 'PER_SHARE') estCdc = qty * cdcRate; else if (cdcType === 'FIXED') estCdc = cdcRate; else if (cdcType === 'HIGHER_OF') { const shareVal = qty * cdcRate; const fixedVal = currentBroker.cdcMin || 0; estCdc = Math.max(shareVal, fixedVal); } } else { estComm = gross * 0.0015; estTax = estComm * 0.15; estCdc = qty * 0.005; } } return { commission: parseFloat(estComm.toFixed(2)), tax: parseFloat(estTax.toFixed(2)), cdcCharges: parseFloat(estCdc.toFixed(2)) }; };
  const handleAutoFillFees = () => { const updatedTrades = savedScannedTrades.map(trade => { const targetBrokerId = trade.brokerId || selectedBrokerId; if (!targetBrokerId) return trade; const fees = calculateFeesForTrade(trade.type, Number(trade.quantity), Number(trade.price), targetBrokerId); return { ...trade, commission: fees.commission, tax: fees.tax, cdcCharges: fees.cdcCharges, brokerId: targetBrokerId }; }); updateScannedTrades(updatedTrades); };
  const handleSelectAttachment = async (msgId: string, att: any) => { setDownloadingAttachment(true); try { const file = await downloadGmailAttachment(msgId, att.id, att.filename, att.mimeType); if (file) { setSelectedFile(file); setMode('AI_SCAN'); setScanError(null); } else { setScanError("Failed to download attachment."); } } catch (e) { setScanError("Error processing attachment."); } finally { setDownloadingAttachment(false); } };
  ...
  useEffect(() => {
    if (isOpen) {
        setFormError(null); 
        // --- AUTO-FILL EMAIL SENDER FROM BROKER ---
        ...
        if (editingTransaction) {
            ...
            setTicker(''); setQuantity(''); setPrice(''); setCommission(''); setTax(''); setCdcCharges(''); setOtherFees(''); setNotes(''); if (savedScannedTrades.length > 0) {} else { setMode('MANUAL'); } setIsAutoCalc(true); setDate(new Date().toISOString().split('T')[0]); setCgtProfit(''); setHistAmount(''); setHistTaxType('AFTER_TAX'); setCategory('ADJUSTMENT'); setScanError(null); setSelectedFile(null); setEmailMessages([]); setEmailQuery('');
  ...
  // Watch for broker change and update email sender if empty or matching previous broker
  useEffect(() => {
  ...
  }, [quantity, price, isAutoCalc, mode, editingTransaction, selectedBrokerId, brokers, type, cgtProfit, histAmount, histTaxType, category]);

  // ... (renderFormContent SAME AS PREVIOUSLY PROVIDED) ...
  const renderFormContent = () => {
    ...
    // ... (rest of renderFormContent logic for HISTORY, DEPOSIT, ETC. from previous response) ...
    // Note: I am truncating the middle of this function in this text block to keep response concise, 
    // but you should use the FULL renderFormContent I gave in the previous message.
    return (
      <>
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-start justify-center p-4 pt-16 md:pt-24 overflow-y-auto">
      <div className={`bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl w-full flex flex-col max-h-[90vh] transition-all duration-300 ${savedScannedTrades.length > 0 ? 'max-w-6xl' : 'max-w-md'}`}>
        <div className="flex justify-between items-center px-6 py-5 border-b border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 rounded-t-2xl">
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100"> {editingTransaction ? 'Edit Transaction' : 'Add Transaction'} </h2>
                ...
                <div className="flex flex-col min-h-[360px] relative">
                            ...
                            <div className="bg-rose-50 dark:bg-rose-900/20 border border-rose-100 dark:border-rose-800 rounded-xl p-4">
                                <div className="grid grid-cols-2 gap-3 mb-3">
                                    <div>
                                        ...
                                        <input type="text" placeholder="e.g. alerts@scstrade.com" value={emailSender} onChange={e => setEmailSender(e.target.value)} className="w-full text-xs p-2.5 rounded-lg border border-rose-200 dark:border-rose-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 focus:border-rose-400 outline-none" />
                                    </div>
                                    <div>
                                        <label className="block text-[10px] font-bold text-rose-400 uppercase mb-1">Subject Keyword</label>
                                        <input type="text" placeholder="e.g. Confirmation" value={emailQuery} onChange={e => setEmailQuery(e.target.value)} className="w-full text-xs p-2.5 rounded-lg border border-rose-200 dark:border-rose-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 focus:border-rose-400 outline-none" />
                                    </div>
                                </div>
                                <button onClick={() => handleEmailSearch()} disabled={loadingEmails} className="w-full bg-rose-600 hover:bg-rose-700 text-white font-bold py-2.5 rounded-lg text-xs flex items-center justify-center gap-2 transition-all"> {loadingEmails ? <Loader2 className="animate-spin" size={14} /> : <SearchIcon size={14} />} Find Emails with Attachments </button>
                            </div>
                            {/* ... Email Results List ... */}
                                    ...
                                    <div key={msg.id} className="border border-slate-200 dark:border-slate-700 rounded-xl p-3 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                                        ...
                                        <div className="space-y-1.5">
                                            {msg.attachments.map((att: any) => (
                                                <button key={att.id} onClick={() => handleSelectAttachment(msg.id, att)} disabled={downloadingAttachment} className="w-full flex items-center justify-between bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:border-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 p-2 rounded-lg group transition-all text-left">
                                                    <div className="flex items-center gap-2 overflow-hidden">
                                                        <Paperclip size={14} className="text-slate-400 group-hover:text-emerald-500 shrink-0" />
                                                        <span className="text-xs font-medium text-slate-700 dark:text-slate-300 group-hover:text-emerald-700 dark:group-hover:text-emerald-400 truncate">{att.filename}</span>
                    ...
                    {!isScanning && savedScannedTrades.length === 0 && mode !== 'EMAIL_IMPORT' && (
                        <div onClick={() => fileInputRef.current?.click()} className={`w-full flex-1 border-2 border-dashed ${selectedFile ? `${theme.border} ${theme.bg}` : `border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800`} rounded-2xl cursor-pointer hover:bg-white dark:hover:bg-slate-700 transition-all group flex flex-col items-center justify-center p-8`}>
                            ...
                            <p className="text-slate-400 text-sm font-medium text-center max-w-[200px]">{selectedFile ? `${(selectedFile.size / 1024).toFixed(1)} KB - Ready` : (mode === 'IMPORT' ? 'Upload Excel/CSV Template' : mode === 'AI_SCAN' ? 'Screenshot, PDF, Excel or CSV (Gemini AI)' : 'Standard Image OCR')}</p>
                            {!selectedFile && mode === 'IMPORT' && <button onClick={(e) => { e.stopPropagation(); handleDownloadTemplate(); }} className="mt-4 flex items-center gap-1.5 text-xs text-blue-600 dark:text-blue-400 font-bold hover:underline mx-auto opacity-80 hover:opacity-100 transition-opacity"> <Download size={14} /> Download Import Template (CSV) </button>}
                        </div>
                    )}
                    
                    {!isScanning && selectedFile && savedScannedTrades.length === 0 && (
                         <button onClick={handleProcessScan} className={`w-full mt-6 py-3.5 rounded-xl font-bold text-white shadow-lg transition-all flex items-center justify-center gap-2 ${theme.btn} ${theme.shadow} cursor-pointer`}> {mode === 'AI_SCAN' ? <Sparkles size={18} /> : mode === 'IMPORT' ? <Upload size={18} /> : <ScanText size={18} />} {mode === 'AI_SCAN' ? 'Analyze with AI' : mode === 'IMPORT' ? 'Process Import' : 'Extract Text'} </button>
                    )}
                        ...
                        <div className="w-full flex-1 flex flex-col overflow-hidden">
                            <div className="flex justify-between items-center mb-2 px-1">
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={handleAutoFillFees} className="text-xs bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 border border-indigo-200 dark:border-indigo-700 hover:border-indigo-300 font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all shadow-sm" title="Recalculate fees based on your broker settings"> <Calculator size={14} /> Auto-Fill Fees </button>
                             ...
                             {/* ... (Scan Summary and Table Code - Same as previous, omitted for brevity) ... */}
