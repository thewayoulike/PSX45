...
export { firstBrokerHolding } from '../utils/brokerHolding';
...
export const TransferModal: React.FC<TransferModalProps> = ({
  ...
  const [convertTax, setConvertTax] = useState<number | ''>('');
  ...
  const taxNum = Math.max(0, Number(convertTax) || 0);

  const estProceeds = qtyNum > 0 && sellNavNum > 0 ? qtyNum * sellNavNum : 0;

  const netProceeds = Math.max(0, estProceeds - taxNum);

  const suggestedDestUnits = netProceeds > 0 && buyNavNum > 0 ? roundFundUnits(netProceeds / buyNavNum) : 0;
              ...
              <div className="rounded-2xl border border-violet-200/70 dark:border-violet-500/20 bg-violet-50/40 dark:bg-violet-500/5 p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  ...
                  <div className="col-span-2 sm:col-span-1">
                    ...
                    <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Tax / CGT</label>
                    ...
                    <input
                      ...
                      onChange={(e) => { setConvertTax(Number(e.target.value)); destUnitsManual.current = false; }}
                ...
                {estProceeds > 0 && (
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 tabular-nums ml-1 space-y-0.5">

                    <span className="block">Gross proceeds ≈ Rs. {estProceeds.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>

                    {taxNum > 0 && (

                      <span className="block">After tax ≈ Rs. {netProceeds.toLocaleString(undefined, { maximumFractionDigits: 2 })} available to subscribe</span>
