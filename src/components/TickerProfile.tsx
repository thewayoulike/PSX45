...
import { ArrowLeft, TrendingUp, TrendingDown, Wallet, Briefcase, PieChart, History, Coins, BarChart3, Rocket, LayoutDashboard } from 'lucide-react';
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
import { MoonshotCalculator } from './MoonshotCalculator'; // IMPORT THE NEW COMPONENT
...
export const TickerProfile: React.FC<TickerProfileProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'OVERVIEW' | 'MOONSHOT'>('OVERVIEW');
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-50 overflow-y-auto animate-in slide-in-from-right duration-300">
      ...
      {/* HEADER */}
      <div className="bg-white/90 backdrop-blur-md sticky top-0 z-40 border-b border-slate-200 px-6 py-4 flex flex-col gap-4 shadow-sm">
         ...
         <div className="flex gap-2">
            <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'OVERVIEW' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:bg-slate-100'}`}
            >
                <LayoutDashboard size={16} /> Overview
            </button>
            <button 
                onClick={() => setActiveTab('MOONSHOT')}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'MOONSHOT' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'text-slate-500 hover:bg-slate-100'}`}
            >
                <Rocket size={16} /> Moonshot Analysis
            </button>
         </div>
      </div>
              ...
              <div className="max-w-4xl mx-auto animate-in fade-in duration-500">
                  <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-6 mb-8 text-center">
                      <h2 className="text-xl font-bold text-indigo-900 mb-2 flex items-center justify-center gap-2">
                          <Rocket size={24} /> Multibagger Scorecard
                      </h2>
                      <p className="text-indigo-700 text-sm max-w-xl mx-auto">
                          Score <strong>{ticker}</strong> against 15 fundamental criteria to determine if it has "Moonshot" potential. 
                          Select the best-fit description for each metric below.
                      </p>
                  </div>
                  <MoonshotCalculator />
              </div>
