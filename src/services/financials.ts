...
// --- FETCH SPECIFIC COMPANY PAYOUTS ---
...
// --- FETCH MARKET WIDE DIVIDENDS ---
export const fetchMarketWideDividends = async (): Promise<CompanyPayout[]> => {
  const targetUrl = `https://www.psx.com.pk/psx/announcement/financial-announcements`;
  const proxies = getProxies(targetUrl);
    ...
    try {
      ...
      if (html && html.length > 500) {
        ...
        rows.forEach(row => {
            const cols = row.querySelectorAll('td');
            if (cols.length >= 8) {
                const companyName = cols[0].textContent?.trim() || 'Unknown';
                const financialResult = cols[2].textContent?.trim() || '-';
                const dividendInfo = cols[3].textContent?.trim() || '-';
                const bookClosureRaw = cols[8].textContent?.trim() || '-';

                if (!dividendInfo || dividendInfo === '-' || dividendInfo.toLowerCase().includes('nil')) return;

                let isUpcoming = false;
                if (bookClosureRaw.includes('-')) {
                    const [startStr] = bookClosureRaw.split('-');
                    const parts = startStr.trim().split('/');
                    if (parts.length === 3) {
                        const bookStart = new Date(parseInt(parts[2]), parseInt(parts[1]) - 1, parseInt(parts[0]));
                        if (bookStart >= today) isUpcoming = true;
                ...
                if (isUpcoming) {
                    payouts.push({ ticker: companyName, announceDate: 'Recent', financialResult, details: dividendInfo, bookClosure: bookClosureRaw, isUpcoming: true });
                }
            }
        });
        return payouts;
      }
    } catch (e) { console.warn(`Proxy ${proxyUrl} failed for market scan`, e); }
  }
  return [];
};
